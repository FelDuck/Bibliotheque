...
class Ball:
    ...
    def is_this_you(self, positions: list, nFrame):
        #Fonction verifie si la liste de point et de pixels de bruit contienne en fait une vrai balle et retourne vrai si ou faux si non
        ...
        # Loop utilise le fait que la balle devrait apparaitre en la meme position deux fois
        # une fois comme nouvelle position et une fois comme apres image laisser par les pixels qui on ete modifier entre frame et frame_old
        ...
        if len(speed) > REPEAT_SPEED_NEEDED:
            condition = False
            condition, speed = self.checkSpeed(speed)
            if(not condition):
                return False

            #estime le temps que la balle sera au marbre avec la vitesse moyenne
    ...
    def checkpositions(self, positions: list, flag=False):
        #Verifie que la position des balles est bonne lors de l'initialisation
        #flag est vrai lors de la premiere serie de ''balle'' lors de la construction de ball
    ...
    def checkSpeed(self,speed):
        """
        Fonction qui verifie si les vitesses sont semblable entres eux
        Utilise une proprieter physique comme quoi la vitesse lateralle est constante si on neglige la resistance de l'air
        marcherait encore mieux avec une vrai balle de baseball

        :param speed: une liste de vitesse
        :return: true false et une liste de vitesse
        """
